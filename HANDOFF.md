...
| companions オトモ | ②図鑑 | 81 | 種名・色名で表示 | 72/81 | scripts/build_critters.py（Fandom Critters記事）。旧「動物」14種は種ごとに統合済み |
...
- キャラ: **なし（72/72 取得済み）**。
- オトモ: **9体**（新規追加分）＝ ペガサス4色 / ガチョウ（ベーシック）/ スイートなハチ4種。Fandom に画像が無い or 未取得。別ソースがあれば `icon_path` に追加。
...
- **build_critters.py**: 動物14種＋オトモ68体（色違い）＋画像。`Critters`記事の餌付け表。SPECIES_WORDSに Cobra/Owl/Pegasus/Baby Dragon/Raven を含める（不規則複数 Pegasi 対応済み）。※その後、新オトモ（ペガサス色違い/ガチョウ/スイートなハチ 等）を手動追加し現在オトモは計81体。
