...
| companions オトモ | ②図鑑 | 196 | 種名・色名で表示 | 159/196 | scripts/build_critters.py（Fandom Critters記事）。旧「動物」14種は種ごとに統合済み。カートゥーン/新種ハリネズミ/イベント・スターパス・プレミアム・キャラのペット等を追加。特別個体は入手元を「生息地」欄に記載（好物なし）。多くの日本語名は暫定（name_en が正） |
...
- キャラ: **なし（72/72 取得済み）**。
- オトモ: **37体**（Fandom に画像が無い/名称不一致で未取得）。159体は Fandom API 経由で取得済み。別ソースがあれば `icon_path` に追加。
