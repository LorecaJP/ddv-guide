...
ddv-guide/
  src/
    ...
    lib/
      schema/index.ts           14カテゴリのTS型 ＋ CATEGORIES（メニュー定義）
    ...
    routes/
      Hub.svelte                トップ（④ハブ＆スポーク＝14カテゴリメニュー）
...
| materials 素材 | 表 | 180 | 148/180 | – | build_materials_json.py＋build_name_ja.py。**種別(category)で13分類**（農作物/フルーツ/穀物/魚/魚介/肉/卵・ナッツ/乳製品/キノコ/茶葉/甘味料/スパイス・ハーブ/その他）。旧「農作物」カテゴリを統合し、栽培場所は obtain_method に取り込み |
...
- 現状: 14カテゴリ稼働、型チェック0エラー、iPhone/iPad/Mac対応、PWA、画像つき公開。
