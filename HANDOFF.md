...
| companions オトモ | ②図鑑 | 196 | 種名・色名で表示 | 72/196 | scripts/build_critters.py（Fandom Critters記事）。旧「動物」14種は種ごとに統合済み。カートゥーン/新種ハリネズミ/イベント・スターパス・プレミアム・キャラのペット等を追加。特別個体は入手元を「生息地」欄に記載（好物なし）。多くの日本語名は暫定（name_en が正） |
...
- キャラ: **なし（72/72 取得済み）**。
- オトモ: **124体**（野生の色違い一部＋特別/イベント/プレミアム/キャラのペットは画像未取得）。72体のみ画像あり。別ソースがあれば `icon_path` に追加。
