...
ddv-guide/
  src/
    ...
    lib/
      schema/index.ts           16カテゴリのTS型 ＋ CATEGORIES（メニュー定義）
    ...
    routes/
      ...
      CharactersZukan/Companions Zukan/AnimalsGrid   ②図鑑グリッド（画像）
      RecipesTable/MaterialsTable                     専用テーブル
      DataTable.svelte          汎用テーブル（tableConfig 駆動：crops/prices/quests/facilities/events/expansions/updates/bugs）
      ArticleList / LinkList     記事型・参照リンク集
...
## 2. カテゴリとデータの現状（16カテゴリすべて稼働）

| カテゴリ | 表示 | 件数 | 日本語名(name_ja) | 画像 | データ源/生成 |
|---|---|---|---|---|---|
| characters キャラ | ②図鑑 | 69 | 69/69 ✅ | 43/69 | scripts/build_characters_json.py（Fandom） |
| companions オトモ | ②図鑑 | 68 | 0（英名） | 64/68 | scripts/build_critters.py（Fandom Critters記事） |
| animals 動物 | カード | 14 | 14/14 | 13/14 | 同上 |
