...
| companions オトモ | ②図鑑 | 95 | 種名・色名で表示 | 72/95 | scripts/build_critters.py（Fandom Critters記事）。旧「動物」14種は種ごとに統合済み。カートゥーン/新種ハリネズミ/イベント個体等を追加 |
...
### 画像が未取得のもの（プレースホルダ 👤/🐾 表示）
- キャラ: **なし（72/72 取得済み）**。
- オトモ: **23体**（新規追加分）＝ ペガサス系5色 / ガチョウ1 / スイートなハチ4 / カートゥーン3（カピバラ・コブラ・サル）/ ドラゴン（ティール）/ ハリネズミ4色 / キツネ（レインボー・ゼロ）/ 古代のロボット / チョウ2。Fandom に画像が無い or 未取得。別ソースがあれば `icon_path` に追加。
...
- **build_critters.py**: 動物14種＋オトモ68体（色違い）＋画像。`Critters`記事の餌付け表。SPECIES_WORDSに Cobra/Owl/Pegasus/Baby Dragon/Raven を含める（不規則複数 Pegasi 対応済み）。※その後、新オトモ（ペガサス色違い/ガチョウ/スイートなハチ/カートゥーン/ハリネズミ/イベント個体 等）を手動追加し現在オトモは計95体。
...
### その後の主な変更（2026-07 時点）
- **構造整理**: 旧「動物」カテゴリを廃止しオトモ図鑑へ統合（→ 15カテゴリ）。到達不能な死コード（ArticleList 等）を除去。表示種別を `image-grid/table/links/dashboard` に整理。
