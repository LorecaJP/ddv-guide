<script lang="ts">
  ...
  // 汎用カタログ表（素材/クラフト素材/花/砂時計 で共有）。
  // 種別(category)ごとにグループ化し各種別内を五十音順、世界(realms)で絞り込み。
  let {
    storeKey,
    title,
    sub = '',
  ...
  $effect(() => {
    setParams(storeKey, { q: query, status: statusFilter, cat: catFilter, realm: realmFilter })
  })

  async function load() {
    loading = true
    await seedAll()
    all = await getAll<CatalogItem>(storeKey)
    loading = false
  }
  load()
  ...
  // 名前を「本体」と「(色)」に分割（(色)は改行で丸ごと2行目へ送る。全角/半角括弧対応）
  ...
  const cats = $derived((() => {
    ...
    const ordered = catOrder.filter((c) => present.has(c))
    const extra = [...present].filter((c) => !catOrder.includes(c)).sort()
  ...
  const groups = $derived((() => {
    ...
    // 五十音順。「きらめく○○」は元の「○○」の直後に並べる（該当が無ければ通常の五十音）
    const isShiny = (m: CatalogItem) => m.name_en.startsWith('Shiny ')
    const baseEn = (m: CatalogItem) => m.name_en.replace(/^Shiny /, '')
  ...
  async function toggleUnlocked(m: CatalogItem) {
    m.unlocked = !m.unlocked
    await put(storeKey, $state.snapshot(m))
    all = [...all]
  }
...
<div class="head">
  <h1>{title}</h1>
  <p class="sub">{all.length} 種 ・ {ownLabel}済み {unlockedCount} 種{sub ? `（${sub}）` : ''}</p>
</div>

<div class="controls">
  <input class="search" type="search" placeholder="{title}名で検索" bind:value={query} />
  <div class="row2">
    {#if realms.length > 1}
    ...
    <select bind:value={statusFilter} aria-label="{ownLabel}状態で絞り込み">
      <option value="all">{ownLabel}：すべて</option>
      <option value="unlocked">{ownLabel}済み</option>
      <option value="locked">未{ownLabel}</option>
    </select>
  </div>
</div>
        ...
        {#each g.items as m (m.id)}
          ...
          <li class="item" class:done={m.unlocked}>
            <div class="line">
              ...
              <button class="own" class:on={m.unlocked} onclick={() => toggleUnlocked(m)} title="{ownLabel}トグル">
                {m.unlocked ? '✓' : '—'}
              </button>
            </div>
