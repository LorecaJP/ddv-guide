<script lang="ts">
  ...
  $effect(() => {
    setParams('snippets', { q: query, cat: catFilter, status: statusFilter })
  })
  ...
  async function load() {
    loading = true
    await seedAll()
    all = (await getAll<Snippet>('snippets')).sort((a, b) => a.name_ja.localeCompare(b.name_ja, 'ja'))
    loading = false
  }
  load()
  ...
  async function save(m: Snippet) {
    await put('snippets', $state.snapshot(m))
    all = [...all]
    if (selected?.id === m.id) selected = m
  }
  function toggleOwned(m: Snippet) {
    m.owned = !m.owned
    save(m)
  }
</script>
...
<div class="head">
  <div>
    <h1>紙精図鑑</h1>
    <p class="sub">{ownedCount} / {all.length} 種 捕獲済み（物語の谷でロイヤルネットで捕まえる紙の生き物）</p>
  </div>
</div>
...
<div class="controls">
  ...
  {/if}
  <select bind:value={statusFilter} aria-label="捕獲状態で絞り込み">
    <option value="all">捕獲：すべて</option>
    <option value="owned">捕獲済み</option>
    <option value="unowned">未捕獲</option>
  </select>
</div>
...
<style>
  ...
  .facts { display: grid; grid-template-columns: 96px 1fr; gap: 10px 12px; margin: 4px 0 18px; font-size: 14px; align-items: start; }
